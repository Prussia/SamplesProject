...
if(!(box.is("select"))){//add by liyt select控件跳过获取焦点绑定事件，防止ie下不好选的情况
...
var needValidCount = _450.length;//add by liyt
$.each(_450,function(){
	if(needValidCount > 0 && $(this).is(":hidden")){
		needValidCount --;
	}
});//end
_450.filter(":not(:disabled):first").focus();
//return _450.length==0;
return needValidCount == 0; //changed by liyt
}
return true;
};
