package com.infotop.app.interceptop.web;
...
/**
 * SysLogAction
 * $Id: SysLogAction.java,v 0.0.1 2015-11-03 15:57:48  $
 */
@Controller
...
public class SysLogController extends BasicController {
	...
	public DataGrid findList(
		...
		try {
			...
			if (user != null) {
				...
				dataGrid = syslogService.dataGrid(searchParams, pageNumber,
						rows, sortType, order);
			} else {
				logger.log(this.getClass(),Logger.ERROR_INT,"登陆帐号无效!","",null);
			}
	 ...
	 public String createForm(Model model) {
	     ShiroUser su = super.getLoginUser();
			...
			if (user != null) {
			     ...
			     model.addAttribute("syslog", entity);
			     model.addAttribute("action", "create");
			} else {
				logger.log(this.getClass(),Logger.ERROR_INT,"登陆帐号无效!","",null);
				return "redirect:/login";
			}
	     return "syslog/syslogForm";
	 }
	 ...
	 */
	 ...
	 @ResponseBody
	 public Message create(@Valid SysLog syslog, RedirectAttributes redirectAttributes) {
		try {
			ShiroUser su = super.getLoginUser();
			User user = accountService.findUserByLoginName(su.getLoginName());
			if (user != null) {
	 ...
	 */
	 ...
	 @ResponseBody
	    public Message update(@Valid @ModelAttribute("preloadSysLog") SysLog syslog,
	 ...
	 */	
	 ...
	 public String view(@PathVariable("id") Long id, Model model) {
		...
		if (user != null) {
	 	...
	 	return "syslog/syslogView";
	 }
	 
	 
	 @ModelAttribute("preloadSysLog")
     public SysLog getSysLog(@RequestParam(value = "id", required = false) Long id) {
        if (id != null) {
